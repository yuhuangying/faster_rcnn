...
from net.fasterrcnn import _fasterRCNN
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
    ##_make_layer由两部分构成，第一部分用来指定是否需要下采样，即每个模块的第一个Bottleneck，第二部分是重复多次的Bottleneck，用来增加模块深度
    # 参数中block为Bottleneck子结构，planes为通道数，blocks为[3,4,6,3]里的某个数字
...
class resnet(_fasterRCNN):
    ...
    def _init_modules(self):
        ...
        # Build resnet.
        #从resnet网络中截取下采样为原图1/16时候的feature map，作为RPN的输入层
        self.RCNN_base = nn.Sequential(resnet.conv1, resnet.bn1,resnet.relu,
            resnet.maxpool,resnet.layer1,resnet.layer2,resnet.layer3)
        #从resnet网络中截取下采样为原图1/32时候的feature map，作为Fast Rcnn分类和回归的的输入层
